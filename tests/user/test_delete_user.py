# # tests/user/test_delete_user.py
...
#
#     @pytest.fixture
#     def new_guest_token(self, api_client, app_config):
#         """Создать новый гостевой токен для этого теста"""
#         response = post_guest_login(api_client, app_config)
#         check_status(response, 200)
#         token = response.json()["token"]
#
#         yield token
#
#         # Cleanup не требуется, т.к. пользователь удаляется в тесте
#
#     @allure.title("Delete user account successfully")
#     @allure.description("Authorized user should be able to delete their account")
#     def test_delete_user_success(self, api_client, app_config, new_guest_token):
#         """Успешное удаление пользователя"""
#
#         with allure.step("Delete user account"):
#             response = delete_user(api_client, app_config, new_guest_token)
#
...
#     def test_deleted_user_cannot_access(self, api_client, app_config, new_guest_token):
#         """Удалённый пользователь не может получить доступ к protected endpoints"""
#
#         # Удалить пользователя
#         delete_user(api_client, app_config, new_guest_token)
#
#         # Попытаться получить информацию о пользователе (должно вернуть 401)
#         with allure.step("Try to access protected endpoint with deleted user token"):
#             api_client.set_token(new_guest_token)
#             response = api_client.get(Endpoints.GET_USER, headers=app_config["headers"])
#
